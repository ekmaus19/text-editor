...
class RichEditor extends React.Component {
  ...
  .then(json => {
    console.log('----------->', json)
    if (json.success) {
      const lastSave = convertFromRaw(JSON.parse(json.document.history[json.document.history.length - 1]))
...
onChange(editorState) {
  this.setState({ editorState }, () => {
    this.state.socket.emit('syncDocument', {
...
onSelection() {
  ...
  handleSaveButton() {
    ...
    fetch(url + '/dashboard/' + this.props.match.params.docId + '/save', {
      method: 'POST',
      credentials: 'same-origin',
      body: JSON.stringify({
